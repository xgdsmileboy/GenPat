 ...
 * @date: 2018/9/21
 */
public class LabeledStmt extends Stmt implements Serializable {
	...
	public void deepMatch(Node other) {
		_tarNode = other;
		if(other instanceof LabeledStmt) {
			_matchNodeType = _oriNode.toString().equals(((LabeledStmt) other)._oriNode.toString());
		} else {
	...
	public boolean matchSketch(Node sketch) {
		if(sketch instanceof LabeledStmt) {
			((LabeledStmt) sketch)._binding = this;
			_binding = sketch;
			return true;
		}
		return false;
	...
	public Node bindingNode(Node patternNode) {
		if(patternNode instanceof LabeledStmt) {
			return this;
		} else {
			return null;
		}
